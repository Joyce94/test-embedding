...
def vector_loader(text_field_words_dict, set_padding=False):
    path = 'word_embedding/glove.sentiment.conj.pretrained.txt'
    ...
    count = 0
    dict_cat = []
    for word in text_field_words_dict.keys():
        if word in words_dict:
            count += 1
            dict_cat.append(words_dict[word])
        else:
            ...
            count += 1
            count_list2.append(count - 1)
    count_data = len(text_field_words_dict) - len(count_list2)
